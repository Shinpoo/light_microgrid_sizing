...
if simple_sizing:
    ...
    load_data = pd.read_csv("data/load.csv", sep=";", parse_dates=True, engine="python", index_col='DateTime')
    ...
    for c in companies:
        ...
        for l in range(len(peak)):  # to replace by len(l_p)
            ...
            for i in range(len(locations)):  # locations
                print(c, locations[str(i)], peak[l])
                ...
                db_ml["longitude"].append(geo_location.longitude)
                db_ml["latitude"].append(geo_location.latitude)
                # db_ml["elevation"].append(elevation)
                db_ml["peak_load"].append(peak[l])
                ...
                db_ml["avg_peak_autumn"].append(load_data.loc[load_data['Season'] == 4].nlargest(
                    int(len(load_data.loc[load_data['Season'] == 1]) * 0.2), [case_name])[
                                                    case_name].mean())
                db_ml["location"].append(locations[str(i)]["location"])
                ...
                if not os.path.exists("results/"):
                    os.makedirs("results/")
                db_ml_df.to_csv("results/mg_sizing_dataset.csv", sep=';',
