...
class SimulatorOptimizer:
    ...
    def _create_sets(self):
        ...
        if self.sizing_config.simulation_step == 60.0:
            if self.use_365_days:
                self.working_series = self.timeseries.resample('60T').mean()
            else:
                self.working_series = self.extracted_series[0].resample('60T').mean()
            print("INFO : working with a time step of 1h")
        else:
    ...
    def optimize_function(self):
        ...
        Bat_reinv_cap = [0.0] * self.sizing_config.investment_horizon
                ...
                if s.name == device_name:
                    ...
                    if self.sizing_config.multi_stage_sizing:
                        for n in range(self.sizing_config.investment_horizon):
                            ...
                            Bat_reinv_cap[n] += self.model.reinv_storages_capacity[s, n].value
            for h in self.entity.h2_storages:
        ...
        print("BESS reinv  = %s kWh" % Bat_reinv_cap)
        ...
        self.optimal_sizing["BATRVSTHORIZON"] = Bat_reinv_cap
